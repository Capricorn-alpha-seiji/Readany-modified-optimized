...
export function LANSyncDialog({ open, onClose, mode }: LANSyncDialogProps) {
  ...
  const ensureNetworkEnabled = useCallback(() => {
    if (networkEnabled) return true;
    toast.message(t("settings.networkDisabledTitle", "联网已关闭"), {
      description: t("settings.networkDisabledHint", "请在设置的通用页面开启联网权限后再使用在线服务。"),
    });
    return false;
  }, [networkEnabled, t]);
  ...
  const handleStartServer = useCallback(async () => {
    if (!ensureNetworkEnabled()) return;
    setError("");
  ...
  }, [ensureNetworkEnabled, isManualIPError, normalizeServerError]);
  ...
  }, [ensureNetworkEnabled, manualServerIP, normalizeServerError, t]);
  ...
  }, [manualIP, manualPort, manualPairCode, ensureNetworkEnabled, confirmLanImport, onClose, syncWithBackend, t]);
