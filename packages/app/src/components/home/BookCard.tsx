...
export const BookCard = memo(function BookCard({
  ...
  return (
    <div
      className="motion-clickable motion-hover-lift group relative flex h-full cursor-pointer flex-col"
      onClick={handleOpen}
      onKeyDown={(event) => {
    ...
    >
      ...
      <div
        ref={coverRef}
        className="book-cover-shadow relative flex aspect-[28/41] w-full items-end justify-center overflow-hidden rounded group-hover:book-cover-shadow"
      >
        ...
        {progressPct > 0 && progressPct < 100 && (
          <div className="absolute bottom-0 left-0 right-0 z-10 h-0.5 bg-black/10">
            <div
              className="motion-scale-progress h-full bg-primary/80"
              style={{ "--motion-progress": progressPct / 100 } as CSSProperties}
            />
          </div>
        )}
        ...
        <button
          ref={menuBtnRef}
          type="button"
          className="motion-pressable absolute right-1 bottom-1 z-20 rounded-md bg-black/30 p-0.5 opacity-0 backdrop-blur-sm group-hover:opacity-100"
          onClick={(e) => {
            e.stopPropagation();
            suppressOpenUntilRef.current = Date.now() + 300;
            if (showMenu) {
      ...
      {showMenu && menuPos && (
        <>
          ...
          />
          <div
            className="readany-floating-motion fixed z-50 min-w-36 rounded-lg border bg-popover p-1 shadow-lg"
            data-state="open"
          ...
          >
            {onShowDetails && (
              <button
                type="button"
                className="motion-pressable flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs text-foreground hover:bg-muted"
            ...
            <button
              id="tour-vectorize"
              type="button"
              className={`motion-pressable flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs ${
            ...
            <div className="relative">
              ...
              {showTagMenu && (
                <div
                  className="readany-floating-motion absolute right-full top-0 z-50 mr-1 min-w-36 max-h-52 overflow-y-auto rounded-lg border bg-popover p-1 shadow-lg"
                  data-state="open"
                  onClick={(e) => e.stopPropagation()}
                  onKeyDown={(e) => e.stopPropagation()}
                >
                  {allTags.map((tag) => {
                    ...
                    return (
                      <button
                        key={tag}
                        type="button"
                        className="motion-pressable flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs hover:bg-muted"
                        onClick={(e) => {
                          e.stopPropagation();
                          suppressOpenUntilRef.current = Date.now() + 300;
            ...
            <button
              type="button"
              className="motion-pressable flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs text-destructive hover:bg-destructive/10"
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <button
              type="button"
              className="motion-pressable inline-flex h-9 items-center justify-center rounded-md border border-border bg-background px-4 text-sm font-medium text-foreground hover:bg-muted"
            ...
            <button
              type="button"
              className="motion-pressable inline-flex h-9 items-center justify-center rounded-md bg-destructive px-4 text-sm font-medium text-destructive-foreground hover:bg-destructive/90"
              onClick={async () => {
                suppressOpenUntilRef.current = Date.now() + 600;
      ...
      <Dialog open={showReindexConfirm} onOpenChange={setShowReindexConfirm}>
        <DialogContent className="max-w-sm" onClick={(e) => e.stopPropagation()}>
          ...
          <DialogFooter>
            <button
              type="button"
              className="motion-pressable inline-flex h-9 items-center justify-center rounded-md border border-input bg-background px-4 text-sm font-medium hover:bg-muted"
              onClick={(e) => {
                e.stopPropagation();
            ...
            <button
              type="button"
              className="motion-pressable inline-flex h-9 items-center justify-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90"
              onClick={(e) => {
                e.stopPropagation();
                setShowReindexConfirm(false);
                doVectorize();
