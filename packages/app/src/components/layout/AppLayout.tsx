...
export function AppLayout() {
  ...
  const removeAppTab = useAppStore((s) => s.removeTab);
  ...
  useEffect(() => {
    ...
    for (const f of customFonts) {
      if (networkEnabled && f.source === "remote" && f.remoteCssUrl) {
    ...
    if (customFonts.every((f) => f.remoteCssUrl || (f.source === "remote" && !networkEnabled))) return;
  ...
  const closeActiveReaderTab = useCallback(() => {
    const activeTab = tabs.find((tab) => tab.id === activeTabId);
    if (!activeTab?.bookId || activeTab.type !== "reader") return false;

    const book = books.find((item) => item.id === activeTab.bookId);
    if (book?.filePath) evictBlobCache(book.filePath);

    removeAppTab(activeTab.id);
    removeReaderTab(activeTab.id);

    const remainingNonHome = tabs.filter((tab) => tab.type !== "home" && tab.id !== activeTab.id);
    if (remainingNonHome.length === 0) {
      setActiveTab("home");
    }

    if (networkEnabled) {
      useSyncStore.getState().syncNow?.().catch((err) => {
        console.warn("[Layout] Failed to sync after closing reader tab:", err);
      });
    }
    return true;
  }, [activeTabId, books, networkEnabled, removeAppTab, removeReaderTab, setActiveTab, tabs]);
  ...
  useEffect(() => {
    const handleIframeKeyDown = (event: MessageEvent) => {
      const data = event.data;
      if (data?.type !== "iframe-keydown" || data.key !== "Escape") return;
      if (data.defaultPrevented || data.isComposing || data.ctrlKey || data.metaKey || data.altKey || data.shiftKey) {
        return;
      }
      const activeTab = tabs.find((tab) => tab.id === activeTabId);
      if (!activeTab?.bookId || activeTab.bookId !== data.bookKey) return;
      closeActiveReaderTab();
    };

    window.addEventListener("message", handleIframeKeyDown);
    return () => window.removeEventListener("message", handleIframeKeyDown);
  }, [activeTabId, closeActiveReaderTab, tabs]);
  ...
  const hideWindowChrome = isReaderActive && !showTabBar;
  ...
  return (
    <div className="readany-motion-root readany-smooth-surface flex h-screen w-screen flex-col overflow-hidden bg-muted">
      {_hasHydrated && <OnboardingModal />}
      <DesktopWindowControls
        className={`readany-tabbar-motion absolute right-0 top-0 z-[80] h-8 bg-muted/95 backdrop-blur ${
          hideWindowChrome ? "-translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
        }`}
      />
      <div
        data-tab-bar
        ...
        className={`readany-tabbar-motion absolute left-0 right-0 top-0 z-50 ${
          hideWindowChrome ? "-translate-y-full opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
        }`}
      >
        <TabBar />
      ...
      <main className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div
          className="readany-view-enter flex min-h-0 flex-1 w-full overflow-hidden"
          style={{ display: !isReaderActive ? "flex" : "none" }}
        >
          <HomeSidebar />
          <div className="min-h-0 flex-1 overflow-hidden pr-1 pb-1">
            <div className="readany-smooth-surface relative flex h-full flex-col overflow-hidden rounded-xl border bg-background shadow-around">
              {/* All home sub-views stay mounted; toggle via display:none */}
              {HOME_VIEWS.map(({ id, Component }) => (
                <div
                  key={id}
                  className={`h-full w-full ${homeViewKey === id ? "readany-view-enter" : ""}`}
        ...
        {readerTabs.map((tab) => {
          ...
          return (
            <div
              key={tab.id}
              className={`absolute inset-0 overflow-hidden ${isActive ? "readany-view-enter" : ""}`}
