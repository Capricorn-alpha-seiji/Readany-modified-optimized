...
export function HomeSidebar() {
  ...
  const handleStatsClick = () => {
    ...
    // Actually, let's keep it simple 鈥?stats is a home sub-view triggered by a special tab id
    setActiveTab("stats");
  };
  ...
  return (
    <aside className="z-40 flex h-full min-h-0 w-48 shrink-0 select-none flex-col overflow-hidden">
      <div className="px-2 pt-2">
        {isSearchVisible ? (
          <div className="flex w-full items-center gap-1.5 rounded-lg bg-muted px-2.5 py-1.5">
          ...
          <button
            type="button"
            className="motion-pressable flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"
            ...
            <div key={item.tabType}>
              {item.expandable ? (
                <div className="flex w-full items-center">
                  <button
                    type="button"
                    className={`motion-pressable flex flex-1 items-center gap-2 rounded-md p-1 py-1 text-left text-sm hover:bg-muted ${isActive ? "text-foreground" : "text-muted-foreground"}`}
                  ...
                  <button
                    type="button"
                    className="motion-pressable flex h-5 w-5 items-center justify-center rounded-full text-muted-foreground hover:bg-muted"
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                <button
                  type="button"
                  className={`motion-pressable flex w-full items-center gap-2 rounded-md p-1 py-1 text-left text-sm hover:bg-muted ${isActive ? "text-foreground" : "text-muted-foreground"}`}
                  onClick={() => handleNavClick(item.tabType)}
                >
                  <Icon size={16} className="shrink-0" />
                  <span className="font-medium text-sm">{t(item.labelKey)}</span>
              ...
              {item.expandable && isLibraryExpanded && (
                <div className="ml-6 mt-1 space-y-0.5">
                  ...
                  <button
                    type="button"
                    className={`motion-pressable flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-xs hover:bg-muted ${
                  ...
                  {/* Group chips 鈥?horizontal, like mobile */}
                  {groups.length > 0 && (
                    <div className="flex flex-wrap gap-1 pt-1">
                      {groups.map((group) => (
                        <button
                          key={group.id}
                          type="button"
                          className={`motion-pressable rounded-full px-2.5 py-0.5 text-[11px] ${
                    ...
                    return (
                      <>
                            ...
                            <div key={tag} className="group/tag relative flex items-center">
                              {editingTag === tag ? (
                                ...
                                <>
                                  <button
                                    type="button"
                                    className={`motion-pressable flex flex-1 items-center gap-1.5 rounded-md px-2 py-1 text-xs hover:bg-muted ${
                                      activeTag === tag
                                        ? "bg-muted text-foreground font-medium"
                                        : "text-muted-foreground hover:text-foreground"
                                    }`}
                                  ...
                                  <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                      <button
                                        type="button"
                                        className="motion-pressable absolute right-0 rounded p-0.5 text-muted-foreground opacity-0 hover:bg-muted hover:text-foreground group-hover/tag:opacity-100"
                        ...
                        {needsCollapse && (
                          <button
                            type="button"
                            className="motion-pressable flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-xs text-muted-foreground/70 hover:bg-muted hover:text-foreground"
                  ...
                  {isAddingTag ? (
                    ...
                    <button
                      type="button"
                      className="motion-pressable flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-muted hover:text-foreground"
