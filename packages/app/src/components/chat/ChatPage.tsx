...
function ThreadsSidebar({
  ...
  return (
    ...
    >
      <div
        className={`readany-toolbar-motion absolute inset-0 ${open ? "bg-black/5 opacity-100" : "opacity-0"}`}
        onClick={onClose}
      />
      <div
        className={`readany-toolbar-motion absolute left-0 top-0 h-full w-72 transform rounded-r-2xl border-r bg-background px-3 py-3 shadow-lg flex flex-col ${open ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"}`}
      >
