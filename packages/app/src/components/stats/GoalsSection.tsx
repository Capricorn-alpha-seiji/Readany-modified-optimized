        ...
        <div className="flex gap-1.5">
          {typeOptions.map((opt) => (
            <button
              ...
              className={cn(
                "motion-pressable flex-1 rounded-lg px-3 py-1.5 text-[12px] font-medium",
                type === opt.key
          ...
          <div key={goal.id} className="group flex items-center gap-5 rounded-xl px-4 py-4 transition-colors hover:bg-muted/[0.08]">
            ...
            {onRemoveGoal && (
              <button onClick={() => onRemoveGoal(goal.id)}
                className="motion-pressable shrink-0 rounded-lg px-2 py-1 text-[11px] text-muted-foreground/52 opacity-0 hover:bg-destructive/10 hover:text-destructive/70 group-hover:opacity-100">
                {copy.removeGoal}
              </button>
            )}
          </div>
