  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      void (async () => {
        ...
        if (
          data?.type !== "iframe-single-click" &&
          data?.type !== "viewer-single-click" &&
          data?.type !== "reader-context-menu"
        ) return;
        if (data.bookKey !== bookKey) return;

        if (data.type === "reader-context-menu") {
          showAndScheduleHide();
          return;
        }
        ...
        if (fraction > leftNavEnd && fraction < rightNavStart) {
          // Middle-zone left click is intentionally neutral; right click opens controls.
          return;
        }
...
export function ReaderView({ bookId, tabId }: ReaderViewProps) {
  const TOOLBAR_PIN_STORAGE_KEY = "readany-reader-toolbar-pinned-v2";
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden rounded-lg border border-border/60 bg-background shadow-sm">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 z-20 h-[2px] bg-foreground/5">
            <div
              className="motion-scale-progress h-full bg-primary/30"
              style={{ "--motion-progress": readerTab?.progress ?? 0 } as CSSProperties}
            />
          </div>
        </div>
        ...
        {showSettings && (
          <>
            ...
            />
            <div className="readany-floating-motion absolute top-12 right-2 z-50 w-80 rounded-lg border border-border/60 bg-background shadow-lg" data-state="open">
